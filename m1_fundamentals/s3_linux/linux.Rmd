...
ls
```
`pwd` prints the working directory, which by default is the directory containing the file we opened, i.e., `linux.Rmd`. `echo` sends what follows in quotes to standard output. The `-e` option allows backslash escapes, which in this case create a new line where specified by `\n`. For reference: `\b` removes spaces between text segments; `\c` suppresses the training new line; `\t` creates horizontal tabs, etc.
...
./start.sh
```
The last command runs the `start.sh` shell script, pulls the pre-built tagged images from Docker Hub (currently 0.9.8), and launches the containers. You must have execute privileges to run the shell script. If not, use `chmod`, e.g., `chmod 755 start.sh`.
