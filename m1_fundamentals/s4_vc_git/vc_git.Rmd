...
We will use the RStudio's builtin `git`, but you should also install `git` on your local computer. `git` is already installed in the `rconnect` container. You will need to activate `git` if it is not already available. Go to `Global Options...` from the `Tools` menu, select Git/SVN from the side panel, and click "Enable version control interface for RStudio projects" and enter the Git executable as `/usr/bin/git` if these are not already specified. You do not need to specify a SSH RSA Key since we will not be using `ssh`.