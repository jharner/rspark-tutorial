...
* supports multi-step data pipelines based on directed acyclic graphs (DAGs);
...
The [SparkR](https://spark.apache.org/docs/latest/sparkr.html) package is another R frontend to Spark, which is officially supported as part of the Spark distribution. However, we will focus on the [sparklyr](https://spark.rstudio.com) package since it ties into the larger tidyverse suite of packages.
...
Using `sparklyr`, connections can be made to local instances or to remote Spark clusters. In our default case the connection is to a local connection bundled in the `rstudio` container. 
