...
### 7.2.2 K-means on the State Crime Data
...
state_crime_pca_proj %>%
  ...
  scale_color_discrete(name = "Predicted Cluster", labels = paste("Cluster", 1:4))
```
This plot agrees with the analysis in Section 7.2.2 except that we reversed the scale of PC2 and the clusters are labeled by different colors. These changes are not relevant to cluster identification.
